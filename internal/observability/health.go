...
func RegisterProbes(r gin.IRoutes, service string, ready func(context.Context) error) {
	...
	r.GET("/readyz", func(c *gin.Context) {
		if ready == nil {
			c.JSON(http.StatusServiceUnavailable, gin.H{"status": "not_ready", "service": service, "error": "no readiness check"})
			return
		}
		ctx, cancel := context.WithTimeout(c.Request.Context(), 2*time.Second)
		defer cancel()
		if err := ready(ctx); err != nil {
			c.JSON(http.StatusServiceUnavailable, gin.H{"status": "not_ready", "service": service, "error": err.Error()})
			return
		}
