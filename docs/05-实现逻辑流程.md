...
| 码 | 典型触发 |
|---|---|
| 40001 | 金额/时间/缺字段/资产未启用/Capture 超额 |
| 40100 | 签名、时间窗、未知 client、query token |
| 40102 | nonce 重放 |
| 40301 | ACL、租户白名单、source_system≠client_id |
| 40401 | 资源不存在 **或跨租户** |
| 40901 | 幂等键冲突但参数变了 |
| 42201 | 余额不足 |
| 42202 | 冻结单非 frozen |
| 42203 | 兑换滑点 |
| 42901 | 单笔/日累计限额 |
| 50000 | Saga 失败/补偿完成等内部 |

---

## 15. 演示账与核对线索

API 启动幂等 `SeedDemo`。检索 `u_alice`：POINT available **7050** / frozen **600**（含一笔过期冻，Worker ~30s 会 Release）；CNY **1049900**；USD **14000**。
