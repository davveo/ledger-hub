...
- `/console`：GET/HEAD 精确路径无 Token 可转发 HTML；其它方法与 `/console/*`、全部 `/api/*` 仍要 Token 或 HMAC
- `/api/*`：`auth → 每 client 秒桶限流 → audit → forward`

```mermaid
flowchart TD
  START[收到请求] --> QTOK{query 含 console_token?}
  QTOK -->|是| R401A[40110 禁止出现在 query]
  QTOK -->|否| PAGE{GET/HEAD 且 path=/console?}
  PAGE -->|是| HTML[转发 HTML 登录页]
  PAGE -->|否| HTOK{Header X-Console-Token 匹配?}
  HTOK -->|是| ROLE{写操作且角色允许?}
  ROLE -->|否| R40314[40314 角色无权]
  ROLE -->|是| CON[注入 X-Console-Role / X-Operator<br/>client_id=console]
...
- Worker 每 15s `ResumeOpenSagas`（跳过 `ResultJSON` 含 `"demo":true` 的演示 Saga）
